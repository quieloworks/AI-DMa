...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!parsed.success)
    return new Response(`${serverT("errors.characterDataInvalid")}: ${parsed.error.message}`, { status: 400 });
  ...
  return new Response(new Uint8Array(pdf), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${(row.name || serverT("characterPdf.filenameFallback")).replace(/[^\w\s-]/g, "")}.pdf"`,
    },
  });
}
