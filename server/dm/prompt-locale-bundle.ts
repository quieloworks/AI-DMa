...
export type DmLocaleBlocks = {
  ...
  combatStrictTurnReminder: string;
};
...
- **PJs en mapa:** cada participante kind \`player\` debe tener \`participants[].id\` **exactamente** igual al \`[id]\` de JUGADORES (mismo string que \`turn_of\` e \`initiative[].player_id\`) para que la app móvil pueda mover la ficha en su turno.
- Opcional: ventana solo de mapa antes de acción/ataque del PJ — usa \`combat_tracker.phase\` \`player_movement\` con el mismo \`turn_of\` activo y sin tiradas obligatorias pendientes (o \`turn_open\` si prefieres una sola fase).
- combat_tracker: con combat:true incluye combat_tracker{round:1,initiative_index:0,turn_of:"(id del primero que actuará tras iniciativa o quien tenga sorpresa/asalto según PHB)",phase:"initiative",note:"…",movement_remaining_feet?} y actualízalo en cada mensaje de combate.
...
- Emite siempre combat_tracker{round,initiative_index,turn_of,phase,note?,movement_remaining_feet?} alineado con la realidad del momento. initiative_index cuenta 0 en el **primero** de la lista INICIATIVA (mayor tirada primero). turn_of = id del combatiente activo (PJ = [id] de JUGADORES; NPC = mismo id que participants[].id). Cuando **turn_of** sea un PJ y **phase** sea \`turn_open\` o \`player_movement\`, incluye **movement_remaining_feet** = pies de movimiento que le quedan en ese turno (típicamente su velocidad al abrir esa fase; la app lo reduce al mover en el mapa).
...
- **Tempo estricto (mesa):** mientras **turn_of** sea el combatiente activo, solo narras y resuelves movimiento, acciones, tiradas y efectos que correspondan a **ese** actor. Prohibido narrar el turno completo de otro (PJ o NPC) en la misma respuesta salvo **reacciones** PHB, lectura breve del entorno que no gaste acciones ajenas, o resoluciones encadenadas del mismo ataque/efecto ya abierto para **turn_of**.
- **Cierre de turno válido:** el jugador puede declarar **pasar turno** explícitamente; o cerraste el turno 5E de ese actor (todas las piezas obligatorias resueltas). Entonces sí avanza initiative_index / cambia turn_of.
- **0 PG inconsciente — salvaciones de muerte:** en el turno en que **turn_of** sea una criatura a 0 PG, limita ese turno a lo PHB: al inicio de ese turno **una** tirada de salvación de muerte (sin movimiento ni acción/acción bonus normales salvo rasgo que lo permita explícitamente). Tras aplicar resultado y estado/HP, cierra ese turno y avanza al siguiente salvo que un efecto muy concreto exija otro paso inequívoco en el mismo turno.
- **Fases phase:** initiative (solo orden); awaiting_dice (salida bloqueada por dados); same_turn_resolution (ya hay resultado de dado y falta aplicar daño/salvación encadenada del **mismo** ataque o efecto); turn_open (el actor puede declarar sin dado obligatorio pendiente); player_movement (opcional: ventana dedicada para que el PJ use el mapa móvil — preparar/confirmar movimiento — antes de acción/ataque; mismo **turn_of**, sin tiradas obligatorias pendientes); between_actors (solo transición breve; el siguiente turno no “gasta” acciones hasta que declares turn_open con el nuevo turn_of).
- **App mapa móvil (PJs):** cuando toque **posición/movimiento** antes de acción o ataque, mantén **phase** en \`turn_open\` o \`player_movement\` (nunca en \`awaiting_dice\` / \`same_turn_resolution\` si el jugador debe poder tocar el mapa). Ritmo sugerido: intención de movimiento/placement → acción → ataque/resolución, sin invitar a otros PJs a gastar turno salvo reacción PHB.
...
- 0 PG: inconsciente; detalle de salvación de muerte en el turno → ver **0 PG inconsciente** arriba; mantén mapa coherente.
...
- Invita a actuar rotando foco (fuera de combate o en turn_open / player_movement del actor que le toca).`,
  continueCombatClock: `
- COMBATE / “Continuar historia”: esto NO es “pasar al siguiente turno de iniciativa” por defecto. Es **avanzar la siguiente fase mecánica** del RELOJ DE COMBATE / combat_tracker: si quedaba daño u otra tirada del **mismo** ataque o del **mismo** turn_of, pédela o resuélvela ahora; actualiza phase (awaiting_dice / same_turn_resolution / turn_open / player_movement / between_actors) y note. Solo incrementa initiative_index y cambia turn_of cuando el turno 5E del actor actual esté **cerrado** (sin dados obligatorios pendientes de ese turno).
...
CD y tipo explícitos. Sensorial breve para {{player}} sin adelantar desenlace de tiradas pendientes.`,
  ...
  combatStrictTurnReminder:
    "- Solo narración y mecánica para **turn_of** hasta cerrar ese turno (salvo reacciones PHB). A 0 PG: turno limitado a salvación de muerte según bloque COMBATE.",
};
...
- **PCs on the map:** each participant with kind \`player\` must have \`participants[].id\` **exactly** equal to their PLAYERS \`[id]\` (same string as \`turn_of\` and \`initiative[].player_id\`) so the mobile app can move the token on their turn.
- Optional: a map-only window before PC action/attack — set \`combat_tracker.phase\` to \`player_movement\` with the same active \`turn_of\` and no mandatory rolls pending (or use \`turn_open\` if you prefer a single phase).
- combat_tracker: when combat:true include combat_tracker{round:1,initiative_index:0,turn_of:"(id of whoever acts first after initiative or surprise/ambush per PHB)",phase:"initiative",note:"…",movement_remaining_feet?} and update it every combat message.
...
- Always emit combat_tracker{round,initiative_index,turn_of,phase,note?,movement_remaining_feet?} matching reality. initiative_index counts from 0 at the **first** entry in INITIATIVE (highest roll first). turn_of = active combatant id (PC = PLAYERS [id]; NPC = same id as participants[].id). When **turn_of** is a PC and **phase** is \`turn_open\` or \`player_movement\`, include **movement_remaining_feet** = feet of movement they have left this turn (typically their speed when that phase opens; the app subtracts when they move on the map).
...
- **Strict table tempo:** while **turn_of** is active, only narrate and resolve movement, actions, rolls, and effects that belong to **that** combatant. Do not narrate another creature’s full turn (PC or NPC) in the same reply except **reactions** per PHB, brief environmental colour that does not spend others’ actions, or chained resolution of the same attack/effect already open for **turn_of**.
- **Valid turn end:** the player may **pass** explicitly; or you finished that actor’s 5E turn (all required pieces resolved). Only then advance initiative_index / turn_of.
- **0 HP unconscious — death saves:** when **turn_of** is a creature at 0 HP, keep that turn PHB-tight: at the start of their turn **one** death saving throw — no normal movement, action, or bonus action unless a feature explicitly allows it. Apply the outcome and state/HP, then close the turn and advance unless a clearly separate rule adds another mandatory step in the same turn.
- **phase values:** initiative (ordering only); awaiting_dice (blocked on dice); same_turn_resolution (dice result exists but chained damage/saves from the **same** attack/effect remain); turn_open (actor can declare with no mandatory roll pending); player_movement (optional: dedicated window for the mobile map — prepare/confirm movement — before action/attack; same **turn_of**, no mandatory rolls pending); between_actors (brief bridge; the next turn does not “spend” actions until you declare turn_open with the new turn_of).
- **Mobile battle map (PCs):** when **position/movement** comes before action or attack, keep **phase** at \`turn_open\` or \`player_movement\` (never \`awaiting_dice\` / \`same_turn_resolution\` if the player must be able to tap the map). Suggested pacing: movement/placement intent → action → attack/resolution, without inviting other PCs to spend their turn except PHB reactions.
...
- 0 HP: unconscious; death-save timing on their turn → see **0 HP unconscious — death saves** above; keep the map honest.
...
- Invite action rotating spotlight (out of combat or on turn_open / player_movement for whoever acts).`,
  continueCombatClock: `
- COMBAT / “Continue story”: this is NOT “advance to the next initiative turn” by default. It is **advance the next mechanical phase** of the COMBAT CLOCK / combat_tracker: if damage or another roll from the **same** attack or **same** turn_of was pending, request or resolve it now; update phase (awaiting_dice / same_turn_resolution / turn_open / player_movement / between_actors) and note. Only increment initiative_index and change turn_of when the current actor’s 5E turn is **closed** (no mandatory dice pending for that turn).
...
DC and type explicit. Brief sensory for {{player}} without advancing outcomes of pending rolls.`,
  ...
  combatStrictTurnReminder:
    "- Narration and mechanics **only** for **turn_of** until that turn closes (except PHB reactions). At 0 HP: turn scoped to death saves per COMBAT block.",
};
